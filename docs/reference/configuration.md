...
`ai_flavor` 的判定依据（不依据答案是否分点/结构化——题目要求分维度作答时，结构化是正常且被期待的）：
...
评卷 LLM 还会输出 `ai_flavor_reason`（AI 痕迹判定依据）。当某题 `ai_flavor >= 1` 时，该依据会拼接到该题「判分理由」末尾，格式为 `；AI痕迹=N（判定依据）`，便于审阅时核对 AI 痕迹打分是否合理。
